...
public class RemoveClinicFromGroupValidatorTests
{
    ...
    {
        var cmd = new RemoveClinicFromGroupCommand(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid());
    ...
    {
        var cmd = new RemoveClinicFromGroupCommand(Guid.Empty, Guid.NewGuid(), Guid.NewGuid());
        var result = _validator.Validate(cmd);
    ...
    {
        var cmd = new RemoveClinicFromGroupCommand(Guid.NewGuid(), Guid.Empty, Guid.NewGuid());
        var result = _validator.Validate(cmd);
        result.IsValid.Should().BeFalse();
