...
public class DismissWelcomeBannerHandlerTests
{
    ...
    [Fact]
    public async Task Handle_WhenOnboardingStateExists_DismissesBannerAndReturnsSuccess()
    {
        // Arrange
        ...
        var handler = new DismissWelcomeBannerHandler(context);
        var command = new DismissWelcomeBannerCommand(userId);
        ...
        saved!.WelcomeBannerDismissed.Should().BeTrue();
    }
    ...
    [Fact]
    public async Task Handle_WhenBannerAlreadyDismissed_StillReturnsSuccess()
    {
        // Arrange
        using var context = BuildContext();
        var userId = Guid.NewGuid();
        var state = CreateOnboardingState(userId);
        state.DismissBanner(); // already dismissed
        context.OnboardingStates.Add(state);
        await context.SaveChangesAsync();
