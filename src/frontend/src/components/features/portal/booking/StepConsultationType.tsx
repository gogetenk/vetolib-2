...
export function StepConsultationType({
  ...
  return (
    <div className="space-y-6" data-testid="step-consultation-type">
      ...
      <div>
        <p className="text-sm font-medium text-stone-700 mb-2">{t('typeLabel')}</p>
        <div
          ...
          {types.map((type) => {
            ...
            return (
              ...
              >
                ...
                <p
                  className={cn('font-semibold text-sm', isSelected ? 'text-emerald-800' : 'text-stone-900')}
                ...
                <div className="flex items-center gap-1 mt-1">
                  ...
                  <span className="text-xs text-stone-400" data-testid={`consultation-type-duration-${type.id}`}>
      ...
      <div>
        ...
        >
          {t('vetLabel')}{' '}
          <span className="font-normal text-stone-400">{t('vetOptional')}</span>
        </label>
        <div className="relative">
          <select
            ...
            className="w-full appearance-none rounded-lg border border-stone-300 bg-white px-3 py-2.5 pr-9 text-sm text-stone-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
          >
      ...
      <div>
        ...
        >
          {t('reasonLabel')}{' '}
          <span className="font-normal text-stone-400">{t('reasonOptional')}</span>
        </label>
        <textarea
          ...
          className="w-full resize-none rounded-lg border border-stone-300 bg-white px-3 py-2.5 text-sm text-stone-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
        />
        <p className="mt-1 text-xs text-stone-400 text-right" aria-live="polite">
          {reason.length}/500
        </p>
      </div>
    </div>
