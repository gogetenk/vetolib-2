...
export function ConversationSummary({ summary, isLoading }: ConversationSummaryProps) {
  ...
  return (
    <div
      className="border border-blue-200 rounded-lg bg-blue-50 mb-4"
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-blue-700">{t('ai_summary')}</span>
        </div>
        {isOpen ? (
      ...
      >
        <div className="px-4 pb-4 text-sm text-blue-900">
          {isLoading ? (
            <p className="italic text-blue-600" data-testid="ai-summary-loading">
              {t('loading_summary')}
            </p>
          ) : summary ? (
            <p data-testid="ai-summary-text">{summary}</p>
          ) : (
            <p className="italic text-blue-600" data-testid="ai-summary-unavailable">
