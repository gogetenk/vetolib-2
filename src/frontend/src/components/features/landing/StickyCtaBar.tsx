...
export function StickyCtaBar({ signupHref, ctaLabel, tagline }: Props) {
  ...
  return (
    <div
      ...
      className={`fixed top-0 left-0 right-0 z-[60] border-b border-emerald-800 bg-emerald-700 transition-all duration-300 ${
        visible
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-2.5 sm:px-6 lg:px-8">
        <p className="hidden text-sm font-medium text-emerald-100 sm:block">
          {tagline}
        </p>
        <Link href={signupHref} data-testid="sticky-cta-link">
          <Button
            size="sm"
            className="bg-white px-6 text-sm font-semibold text-emerald-700 transition-all duration-200 hover:bg-emerald-50 hover:shadow-md"
